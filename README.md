...
Talk to the [HelloAgent](https://helloagent.io) relay from Node or the browser. Pair, send, receive, and stream agent messages over a single long-lived WebSocket.
...
Run an existing agent token? Get one from [https://web.helloagent.io/app/agents/new](https://web.helloagent.io/app/agents/new).
...
The SDK doesn't ship a pairing CLI — that's the OpenClaw plugin's job. To use the SDK directly, get an `ha_*` token from `https://web.helloagent.io/app/agents/new` and feed it to `Agent({ token })`.
